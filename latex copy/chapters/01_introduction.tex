...
substations, so nothing it learns can be reused elsewhere. Fixing both --- first
the training, then the representation --- is the subject of the chapters that
follow.

\section{Outline}
\label{sec:intro-outline}

Chapter~\ref{ch:baseline} tunes that baseline until it trains stably, then
replaces the flat actor with a graph encoder and checks that nothing is lost.
Chapter~\ref{ch:methods} defines the graph inputs: feature scaling, three
representations of the grid, the substation-level augmentations, the readout,
and the candidate-action head. Chapter~\ref{ch:experiments} screens those
choices experimentally. Chapter~\ref{ch:sparse-control} changes the objective
and asks whether the agents can keep their survival while intervening rarely.
Chapter~\ref{ch:wcci-transfer} moves to the 36-substation WCCI grid and tests
