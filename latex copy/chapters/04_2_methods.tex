...
\begin{itemize}
  ...
  a time-dependent mask retains only the relations that are electrically active
  ...
  one-hop neighboring context. The reference architecture uses a two-layer,
...
\textbf{Takeaway:} The graph variants change how the same grid state is
represented and propagated, while the MAPPO training structure and agent
...
\end{equation}
where $m_v=1$ for a valid node and $m_v=0$ otherwise. A single mean is computed
over all node types; there is no separate pooling operation for each type. The
division by the number of valid nodes keeps the scale of the graph embedding
...
\\
\\
For mean or maximum pooling, the pooled set is every valid node in the agent's
local graph. It contains all controlled and contextual busbars, including empty
busbars; all generator and load nodes in the disaggregated schemas, including
disconnected assets; and all transmission-line nodes in the explicit-line
schema. When the substation-summary augmentation is enabled, its summary nodes
are included as well. Direct same-substation edges add no nodes and therefore do
not change the pooled set. Table~\ref{tab:pooled-nodes} gives the node types and
counts for each representation. Virtual-node runs do not use this terminal
...
to all included busbars, or to all included substation-summary nodes when that
...
\begin{itemize}
  ...
  \item \textbf{Terminal pooling:} Mean pooling averages every valid node in the
  local graph, including contextual and added substation nodes. Maximum pooling
  keeps the largest value in each hidden channel. Sum pooling is not used
  because its scale changes with the number of nodes.
