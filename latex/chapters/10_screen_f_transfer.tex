\chapter{\texorpdfstring{Complete-Policy Transfer and Target Adaptation}{Complete-Policy Transfer and Target Adaptation}}
\label{ch:screen-f-transfer}

Chapter~\ref{ch:encoder-transfer-bridge} tested whether a source
...
target training, then studies target adaptation.

\section{Comparison rules}
\label{sec:wcci-design}

The transferred object is the complete candidate-action actor,
not only its graph encoder. The actor varies along four binary choices: original
or transformed inputs, mean or typed-mean candidate pooling, action descriptor
off or on, and a shared or dedicated idle-action head.
Table~\ref{tab:wcci-transfer-design} defines the three training routes.
...
\begin{table}[H]
  ...
  \begin{tabular}{L{0.22\textwidth}L{0.30\textwidth}L{0.38\textwidth}}
    ...
    Fine-tuned & Shared \texttt{bus14} actor & 5M target steps \\
    \bottomrule
  \end{tabular}
\end{table}

\noindent Final checkpoints are used for actors trained on WCCI. Zero-shot evaluation
...
the same action count. A negative value is worse than do nothing. This allows to judge the policy
according the maximum performance empirically found with this action-space size.
...
\begin{figure}[H]
  ...
  contains all 16 architectures across the raw-input and transformed-input
  conditions. Green stars give the greedy full-test score and dashed lines give
  do nothing. The lower-right score measures progress from do nothing to the
  greedy reference at the same action-space size.}
  \label{fig:wcci-zero-shot-determinants}
\end{figure}

\noindent The distributions show that absolute and feasible-relative performance answer
different questions. With the heuristic, the median of the models'
full-test mean-survival scores is $62.5\%$ at $k=32$ and about $54.5\%$ for the
larger action spaces; the median difficult-cohort mean survival across
...
effects by gate condition. Typed-mean candidate pooling is the clearest positive effect:
...
\noindent These comparisons use one seed. They identify useful directions but do not
...
\noindent The absolute view shows whether a positive change is large enough to produce a
...
\begin{figure}[H]
  ...
  \caption{Paired changes on the difficult cohort. In the left panel, each
  policy variant is compared with the plain zero-shot actor using the same
  architecture, action-space size and gate condition. In the right panel,
  local-$\rho=0.95$ evaluation is compared with ungated evaluation for the same
  policy, architecture and action-space size. Points are mean paired
  differences and error bars are $95\%$ bootstrap intervals. Positive values
  favour the first condition named in each panel; zero is the reference.}
  \label{fig:wcci-intervention-gate}
\end{figure}

\noindent Without the heuristic, Gmax-delta adds $4.6$ points and the adaptive budget
...
Figure~\ref{fig:wcci-adaptation} compares the same eight transformed-input
architectures at $k=64$. The left panel tracks each architecture across
zero-shot, scratch and fine-tuned routes. The right panel shows the main
trade-off: mean survival on the difficult cohort against the number of easy
chronics kept.
...
\noindent Fine-tuning is the most consistent route. It beats training from scratch on
...
average. The median difficult-cohort mean survival across architectures rises from $5.7\%$ zero-shot to
...
The study uses one training seed and 50 evaluation chronics, which is not sufficient for generalization. Bootstrap intervals
describe variation across architecture cells, not across independent training
runs. The results support comparisons inside the matched blocks, but small gaps between unrelated
...
\textbf{Purpose:} To determine how well the complete shared
...
measure whether WCCI training benefits from fine-tuning.
\\
\\
\textbf{Content:} Unlike Chapter~\ref{ch:encoder-transfer-bridge},
the source action interface is not replaced. We evaluate 16 architecture cells
across the raw-input and transformed-input conditions over
$k\in\{32,64,128,256\}$, with and without a local-$\rho=0.95$ gate, using both
absolute mean survival and progress from do nothing to the same-$k$ greedy
reference. We then test Gmax-delta and adaptive-budget policy changes before
comparing the same eight transformed-input architectures at $k=64$ under
zero-shot transfer, training from scratch and target fine-tuning.
\\
\\
\textbf{Findings:}
\begin{itemize}
  ...
  \item Typed-mean candidate pooling is the clearest positive architecture effect,
  adding $5.4$ difficult-cohort mean-survival points ungated and $8.7$ gated. Physical
  ...
  cells by $7.9$ points. Median difficult-cohort mean survival across architectures progresses from
...
and typed-mean candidate pooling are the most robust zero-shot choices; local gating must
