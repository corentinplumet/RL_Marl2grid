...
\subsubsection{Local disaggregated graphs}
...
\end{equation}
nodes. A generator or load node remains allocated when it is disconnected, and
...
A disconnected line therefore keeps its node row. Its \texttt{line\_status}
...
For this schema, GINE and GAT receive no line-state value on the attachment
edges. Every endpoint edge contains only one endpoint-and-direction relation
indicator; the complete line state is carried by the transmission-line node.
...
\subsubsection{Local disaggregated-line graphs}
...
not one mean per type. Thus the busbar graph pools busbar nodes, the
...
contextual nodes are included in the ordinary mean readout. Sum, maximum, or
attention pooling can replace the aggregation in
Equation~\ref{eq:methods-mean-pooling} without changing message passing.
