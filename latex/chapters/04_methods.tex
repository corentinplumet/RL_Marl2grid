...
{\color{clarif}
\paragraph{There is no per-asset connection flag.}
An earlier version of this schema carried a \texttt{connected} column, written
as a constant on busbar rows, as a copy of \texttt{line\_status} on line rows,
and as the attachment state on generator and load rows. Only the last of those
carried information, and it never varied: a \emph{change} action toggles an
element between busbars and cannot detach it, and a substation split that
isolated a load or generator ends the episode. Measured over $600$ random
topology actions on \texttt{bus14} spanning $180$ episode ends, and over $400$
on the WCCI grid, no generator or load was ever observed detached, against
$150$ and $493$ line disconnections respectively. The column was therefore
removed rather than carried.
\\
\\
A detached asset would still be identifiable: its measured channels are zeroed
while its type indicator stays set. What is lost is only the distinction
between that state and an attached asset reading zero, which would matter under
an action space exposing \texttt{set\_bus}. Note also that connection in this
sense was never the same thing as energization, which is reported separately by
the energized mask of Section~\ref{sec:graph-pooling}: on agent~$0$ of
\texttt{bus14} four of eight busbars carry no current under the nominal
topology.
}
...
{\color{clarif}
\paragraph{A relation carrying no flow reports no loading.}
An earlier version set \texttt{rho}$=1$ on structural and attachment
relations. That value was a multiplicative identity for the weighted-GCN
variant, which reads the \texttt{rho} column directly as a scalar message
weight and would delete any message it weighted by zero. It was not a
measurement, and for the GINE and GAT encoders used throughout -- which consume
the edge vector as an attribute through a multilayer perceptron and never touch
the weight path -- it made a relation carrying no current read as a line at its
thermal limit.
\\
\\
The attribute is now zero, like every other physical channel of the neutral
block, and the substitution moved to the encoder: the weighted variant takes the
measured loading on physical lines and unit weight on everything else. The
attribute stays an honest measurement for the encoders that read it as one, and
the weighted variant still behaves as intended.
}
