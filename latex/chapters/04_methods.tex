...
Each agent $a$ controls a set of substations $\mathcal{C}_a$ and never sees the
whole grid. Its local graph is built in two steps. First the \emph{region} is
fixed: with one-hop context enabled, which is the setting used throughout, it
contains every line with at least one endpoint in $\mathcal{C}_a$, together with
the substations at both ends of those lines. The region is therefore
$\mathcal{C}_a$ plus a ring of neighboring substations that the agent can see but
cannot act on. Without one-hop context the region is $\mathcal{C}_a$ alone and
only lines internal to it are kept.
\\
\\
The region is a set of substations, but no node of the graph is a substation.
The second step fills the region with the entities that do get nodes, and this
is where the three schemas differ. A substation contributes its $B$ busbar
nodes, one per possible busbar, in every schema; a node standing for the
substation itself exists only when the summary augmentation of
Section~\ref{sec:substation-summary-nodes} is enabled. \emph{Controlled} and
\emph{contextual} below therefore describe where a node sits, not a node type:
a busbar is controlled when its substation is in $\mathcal{C}_a$. Table~\ref{tab:pooled-nodes} states what each one puts in the
graph, with a worked count for one agent.

\begin{table}[H]
  \centering
  \small
  \caption{What each schema puts in one agent's local graph, with one-hop
  context enabled. The example is agent 0 of \texttt{bus14}, which controls
  substations $\{0,1,2,4\}$ and sees substations $\{3,5\}$ as context, with
  $B=2$ busbars per substation and eight lines touching its region.}
  \label{tab:pooled-nodes}
  \setlength{\tabcolsep}{5pt}
  \begin{tabular}{L{0.23\textwidth}L{0.42\textwidth}L{0.24\textwidth}}
    \toprule
    Schema & Node types in the local graph & Example: agent 0 \\
    \midrule
    \texttt{bus} & Busbar nodes only, $B$ for each substation of the region &
    12 busbar nodes, 8 controlled and 4 contextual \\
    \texttt{heterogeneous} & The same busbars, plus one node per generator and
    one per load attached to any substation of the region &
    12 busbars, 5 generators, 5 loads: 22 \\
    \texttt{heterogeneous\_line} & The same busbars and asset nodes, plus one
    node per line of the region & 22 as above, plus 8 line nodes: 30 \\
    \bottomrule
  \end{tabular}
\end{table}

{\color{darkred}
\noindent The readout takes \emph{one} mean over \emph{every} node of that
graph. Nothing is filtered out, and Table~\ref{tab:pooling-summary} lists what
that includes and what it costs.

\begin{table}[H]
  \centering
  \small
  \caption{What the mean readout averages, and the two effects it produces.
  Counts are for agent 0 of \texttt{bus14}.}
  \label{tab:pooling-summary}
  \setlength{\tabcolsep}{5pt}
  \begin{tabular}{L{0.30\textwidth}L{0.58\textwidth}}
    \toprule
    Averaged & Consequence \\
    \midrule
    Busbars of controlled substations & The nodes the agent acts on: 8 of 22 \\
    Busbars of contextual substations & Neighbors it cannot act on are averaged in on equal terms: 4 of 22 \\
    Empty busbars & Half the busbars in the nominal topology; they add rows of zeros \\
    Generator and load nodes, contextual ones included & Only these carry \texttt{p} and \texttt{theta}: 10 of 22 \\
    Disconnected assets & Keep their node, with zeros and \texttt{connected}$=0$ \\
    Line nodes, in \texttt{heterogeneous\_line} only & Add the line state to the same average \\
    \bottomrule
  \end{tabular}
\end{table}

\noindent Two effects follow. Context dilutes: a third of agent 0's busbar
nodes belong to substations it cannot act on, and ten of agent 1's eighteen. And
in the disaggregated schemas a busbar node has \texttt{p} and \texttt{theta}
exactly zero, because those quantities moved to the asset nodes, so the mean of
those columns is the asset total divided by the count of \emph{all} nodes. On
\texttt{bus14} busbars are 55\,\% of the pooled set for agent 0, 64\,\% for
agent 1 and 62\,\% for agent 2, so the same generation reaches each agent's
readout attenuated by a different factor, and by a different one again on
another grid (Section~\ref{sec:wcci-why-distribution}).

\noindent The \texttt{domain\_mask} feature marks which nodes are controlled,
but it is an input column the encoder reads, not a filter on the readout.
}
...
not one mean per type, so that the readout stays comparable across grids of
different composition. The nodes entering that average are every node of the
agent's local graph, listed per schema in Table~\ref{tab:pooled-nodes}.

The hierarchy nodes of Section~\ref{sec:substation-representation}, when
enabled, enter the average as ordinary nodes as well.
\\
\\
Sum pooling is available but is not used in any reported experiment. Its
magnitude grows with the number of pooled nodes, which differs between agents
and between grids, so a readout built from it would not be comparable across the
transfer studied in Chapter~\ref{ch:wcci-transfer}.
\\
\\
