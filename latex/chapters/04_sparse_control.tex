...
\\
\\
Mechanisms are therefore measured on two competing axes: survival rate and
...
\]
This can accidentally reward intervention because a larger \(P(g_i=1)\) allows
...
used and gets little learning signal. For this reason,
...
$\lambda_{fixed}=0.010$ tests whether the gate still helps once a simple sparse
...
\paragraph{Implementation detail.}
The evaluation-time rho heuristics
(Section~\ref{subsec:sparse-control-heuristics}) produce highly sparse, safe
behavior, but they require a manual override. To move this behavior into the
policy weights, a teacher-student distillation approach is tested. The teacher
is a MAPPO model running the local rho heuristic, and the student is trained via
Behavioral Cloning (BC) to match the teacher's executed actions. The student is
then evaluated without the heuristic override, to see whether it learned to copy
the sparsity on its own. Two balancing settings are tested to handle the strong
class imbalance caused by the teacher's high action-0 rate.

\paragraph{Interpretation.}
As reported in Appendix~\ref{app:sparse-control-full-test-tables}, the student
policies reach roughly 91--92\% survival and 94--96\% sparsity. This shows that
sparse behavior can be partly distilled into the actor weights. Still, the
resulting student is weaker than the AIB methods and gives up part of the
teacher's survival (97.1\%). The distillation route is therefore less reliable
than learning the sparse objective directly through the Adaptive Intervention
Budget.
...
\begin{figure}[H]
  ...
  rates and full-test survival, so it shows the chapter's two objectives at
  once.}
  \label{fig:sparse-control-fulltest-tradeoff}
\end{figure}

\paragraph{Interpretation.}
The current reading is that the gate is not the strongest
...
the flat local-safe AIB policy with $d=0.20$ is the best-justified learned
sparse-control candidate. It keeps the original decentralized actor, does not
rely on an evaluation-time override, learns the intervention penalty
...
mode visible: high action-0 usage is not enough when the gate collapses and
survival falls.

\section{Preliminary WCCI Stress Test}
\label{sec:sparse-wcci-stress-test}

As an early scale test, representative mechanisms were retrained from scratch
with an MLP actor on the maintenance-enabled WCCI grid. No \texttt{bus14}
...
\begin{figure}[H]
  ...
  \caption{Action-0/survival trade-off for the early
  maintenance-enabled WCCI runs. Each point is a seed-aggregated run variant.
  The upper-right region is still the target, but no method combines high
...
survival over the weak MLP baseline: the differences are small compared with the
seed variation. The local rho heuristic and AIB are highly sparse but remain
weak controllers, which suggests that sparsity transfers more easily than
control quality. This failed pilot changes grid size, action space, and
maintenance at once, so it cannot identify the cause. It still shows the limit
...
separates the factors that are mixed together here, before studying cross-grid
transfer.
\\
\\
\begin{tcolorbox}[
...
\textbf{Objective:} Keep survival while reducing unnecessary topology
interventions.
\\
\\
\textbf{Findings:}
