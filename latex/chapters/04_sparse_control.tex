...
\noindent \latestrevision{The exact evaluation rules used to convert the two
heads into one action are documented in
...
\paragraph{Interpretation.}
\latestrevision{The intervention gate is not the strongest method by itself.
The fixed penalty with $\lambda_{fixed}=0.010$ already reduces unnecessary
actions, while local-safe AIB with $d=0.20$ gives the best balance between
survival and action-0 usage among the learned methods. It remains decentralized
and allows the agents to act more freely when their local grid is stressed.}
...
next. The full protocol, table, and trade-off figure are retained in
Appendix~\ref{app:sparse-control-wcci-pilot}.}
\\
\\
\begin{tcolorbox}[
