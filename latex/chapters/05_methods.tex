...
per type and concatenates the four results, giving width $4d_h$ and preserving
...
preserving action \(0\); an agent with fewer than $k$ actions keeps all of them.
\\
\\
Choosing $k$ is an empirical question rather than a design one, so it is settled
...
also sizes the reduction, since it shows how much survival the action set can
...
\begin{itemize}
  ...
  and action 0 is always retained; a one-step simulator-greedy controller checks
