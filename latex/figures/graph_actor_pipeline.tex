...
\node[
  ...
  {\bfseries $K\times$ GINE layers}\\[7pt]
...
\node[
  ...
  {\bfseries Masked mean readout}\\[4pt]
  average $\mathbf h^{(K)}$ over the\\
  visible nodes $\Rightarrow\bar{\mathbf h}_a$\\[3pt]
  {\scriptsize \texttt{controlled\_mean}: controlled nodes only}
};
...
\draw[flow] (graphproj.south) -- (14.18,-2.92) -| node[note, near start, right] {$\mathbf z_a\in\mathbb R^{128}$} (actor.north);
