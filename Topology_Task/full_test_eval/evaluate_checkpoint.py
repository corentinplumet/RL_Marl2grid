...
def _saved_node_pre_encoder_width(record: Dict[str, Any]) -> Optional[int]:
    """Return the raw input width expected by a saved node pre-encoder."""
    ...
    for value in record.values():
        ...
        for key, tensor in value.items():
            if not key.endswith("node_pre_encoder.0.weight"):
                continue
            if isinstance(tensor, th.Tensor) and tensor.ndim == 2:
...
def _configure_legacy_connected_feature(
    args: Namespace, record: Dict[str, Any]
) -> Namespace:
    """Rebuild the obsolete ``connected`` node column when weights require it.

    Checkpoints saved before the no-leak node-schema cleanup have one extra
    input column.  Detect that architecture from the saved first layer instead
    of relying on filenames or checkpoint dates.
    """
    ...
    if graph_type in {
        ...
        # Current explicit-line schema: four type flags plus eight physical /
        # structural channels.  Legacy checkpoints add ``connected``.
        current_raw_width = 12
        if _scenario_has_maintenance(args):
            current_raw_width += 2
    else:
        # Current equipment-node schema has seven columns; edge-difference
        # angle representation removes the absolute-angle column.
        current_raw_width = 7
