#!/usr/bin/env bash
# Evaluate all eight WCCI-fine-tuned shared NLS checkpoints on WCCI mk64.
...
result_group="trcas_shared_NLS_finetune_wcci36_mk64"
plan_labels=()
plan_checkpoints=()
plan_outputs=()
plan_action_dirs=()
...
for variant in "${variants[@]}"; do
  ...
  output="outputs/full_test_eval/shared/${result_group}/best_test_${run_name}_mk64.json"
  action_dir="outputs/full_test_eval_actions/shared/${result_group}/best_test_${run_name}_mk64"
...
if [[ $matched -eq 0 ]]; then
  echo "No mk64 fine-tuned evaluations matched filters: $*" >&2
  echo "Examples: mean_f1_a0h0  tmean  a0h1" >&2
  exit 1
fi
...
if [[ ! -f "$action_space_abs" ]]; then
  if is_true "$dry_run"; then
    echo "Dry-run warning: missing mk64 action space locally: $action_space_abs" >&2
  else
...
wrong = {agent: size for agent, size in sizes.items() if size != expected}
if wrong:
    raise SystemExit(
        f"Expected exactly {expected} selected actions per agent in {path}; "
        f"got {sizes}"
    )
configured_caps = {
    agent: int(cap)
...
if configured_caps:
    wrong_caps = {
        ...
        for agent in sizes
        if configured_caps.get(agent) != expected
    }
    if wrong_caps:
        raise SystemExit(
            f"Expected per-agent cap {expected} in {path}; got {configured_caps}"
        )
    if int(reduction.get("top_k", -1)) != expected:
        print(
            f"Warning: global top_k={reduction.get('top_k')} is stale; "
            f"validated the explicit mk{expected} per-agent caps instead."
        )
elif int(reduction.get("top_k", -1)) != expected:
    raise SystemExit(
        f"Expected top_k={expected} in {path}; got {reduction.get('top_k')} "
        "and no explicit top_k_by_agent override."
    )
print(
    f"Validated mk{expected} target action space: "
...
echo "========== NLS fine-tuned WCCI mk64 evaluation =========="
echo "Dataset:             bus36_wcci_nomaint test split"
echo "Episodes per model:  50"
echo "Source checkpoints:  WCCI fine-tuned with mk256"
echo "Evaluation actions:  WCCI mk64"
...
if is_true "$dry_run"; then
  echo "Validated ${#plan_labels[@]} WCCI mk64 submission command(s)."
else
  echo "Submitted ${#plan_labels[@]} WCCI mk64 evaluation job(s)."
fi
