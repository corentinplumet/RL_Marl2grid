#!/usr/bin/env bash
# EPFL JED launcher for dangerous-state graph behavior-cloning fine-tuning.
...
if [[ "${1:-}" == "--help" || "${1:-}" == "-h" ]]; then
    cat <<'EOF'
Usage:
  sbatch Topology_Task/teacher_student/job_train_dangerous_graph_bc_jed.sh \
    ...
    --output checkpoint/dangerous_graph_bc/bcg_cons1496_NLS_mean_f1_a0h0_s0.tar \
    --distill-weight 0 \
    --device cpu
