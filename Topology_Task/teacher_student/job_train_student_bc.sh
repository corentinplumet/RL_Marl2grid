...
echo "Student BC job ${SLURM_JOB_ID:-local} started on $(hostname) at $(date)"
...
echo "Arguments: $*"
python -u teacher_student/train_student_bc.py "$@"

echo "Student BC job ${SLURM_JOB_ID:-local} finished at $(date)"
