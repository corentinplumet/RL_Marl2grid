#!/usr/bin/env bash
# Screen C rerun with the selected GNN architecture, seed 0, on IZAR.
...
config_dir="Topology_Task/configs/nl_s2em_structure"
filters=("$@")
submitted=0
...
echo "$( is_true "${DRY_RUN:-false}" && echo Validated || echo Submitted) $submitted runs for nl_s2em_structure."
