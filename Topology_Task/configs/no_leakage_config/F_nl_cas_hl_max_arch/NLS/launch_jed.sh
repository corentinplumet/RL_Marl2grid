#!/usr/bin/env bash
# Submit the eight-run NLS global-max/action-delta screen on JED.
...
task_config_dir="configs/no_leakage_config/F_nl_cas_hl_max_arch/NLS"
filters=("$@")
submitted=0
jed_time_limit="${TIME_LIMIT:-4260}"
...
if [[ $submitted -eq 0 ]]; then
  echo "No NLS configurations matched: ${*:-<none>}." >&2
  exit 1
fi
echo "NLS configs selected on JED: $submitted."
