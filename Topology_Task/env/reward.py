...
class OverloadReward(BaseReward):
    ...
    def __call__(
        ...
        penalty_disconnection = (
            self.penalty
            * sum(~env.current_obs.line_status)
            / (env.current_obs.n_line * 0.1)
        )
...
class LineMarginReward(BaseReward):
    ...
    def __call__(
        ...
        reward = margin[env.current_obs.line_status].sum() + (
            self.penalty * sum(~env.current_obs.line_status) / env.current_obs.n_line
        )
...
class RedispRewardv1(RedispReward):
    ...
    def __call__(
        ...
        redisp_cost = (
            self._alpha_redisp
            * np.abs(env._actual_dispatch).sum()
            * marginal_cost
...
class N1ContingencyRewardv1(N1ContingencyReward):
    ...
    def __call__(self, action, env, has_error, is_done, is_illegal, is_ambiguous):
        ...
        th_lim_a = 1.0 * env.get_thermal_limit()
        th_lim_a[th_lim_a <= 1.0] = 1.0  # assign 1 for the thermal limit
        ...
        if not conv:
            self.logger.warn(
                "Cannot set the backend of the `N1ContingencyReward` => divergence"
            )
            return self.reward_min
        ...
        self.logger.info(
            f"{contingecy_analyzer.computer.nb_solved()} converging contingencies"
        )
        if self._dc:
            ...
            p_sq = (1e-3 * th_lim_a) ** 2 * 3.0 * vor**2 - qor**2
            p_sq[p_sq <= 0.0] = 0.0
            limits = np.sqrt(p_sq)
        else:
            tmp_res = 1.0 * tmp[1]
            limits = th_lim_a
        res = (
            (tmp_res > self._threshold_margin * limits) | (~np.isfinite(tmp_res))
        ).any(axis=1)  # whether one powerline is above its limit, per cont
        res |= (np.abs(tmp_res) <= self._tol).all(
            axis=1
        )  # other type of divergence: all 0.
